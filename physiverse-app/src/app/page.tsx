...
export default function HomePage() {
  return (
    <>
      ...
      {/* ════════════════════════════════════════════════════
          SECTION 4.5: Explore Inside (Exploded View)
         ════════════════════════════════════════════════════ */}
      <Section id="explore-inside">
        <div className="section-container">
          <motion.div variants={fadeUp} className="text-center mb-14">
            <span className="text-sm font-semibold uppercase tracking-wider" style={{ color: 'var(--color-primary)' }}>
              Exploded View
            ...
            <p className="max-w-2xl mx-auto text-center" style={{ color: 'var(--text-muted)' }}>
              Disassemble scientific instruments like an engineer. Inspect every component,
              learn the physics behind each part, and reassemble them in interactive 3D.
            </p>
          </motion.div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {[
              { id: 'microscope', title: 'Compound Microscope', parts: 16, icon: Microscope, color: '#8B5CF6', category: 'Optics' },
              { id: 'atom', title: 'Floating Atom', parts: 7, icon: Atom, color: '#3B82F6', category: 'Modern Physics' },
              { id: 'gyroscope', title: 'Gyroscope', parts: 6, icon: RotateCcw, color: '#10B981', category: 'Mechanics' },
            ].map((obj) => {
              const Icon = obj.icon;
              return (
                <motion.div key={obj.id} variants={fadeUp}>
                  <Link href={`/explore/${obj.id}`} className="explore-inside-card group block">
                    <div
                      className="relative h-44 flex items-center justify-center"
                      style={{ background: `linear-gradient(135deg, ${obj.color}15, ${obj.color}05)` }}
                    >
                      <Icon
                        className="w-16 h-16 opacity-15 group-hover:opacity-30 group-hover:scale-110 transition-all duration-500"
                        style={{ color: obj.color }}
                      />
                      <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300">
                        <div className="w-14 h-14 rounded-full flex items-center justify-center shadow-xl" style={{ background: obj.color }}>
                    ...
                    <div className="p-5">
                      <div className="inline-block text-[10px] font-semibold px-2 py-0.5 rounded-full mb-2"
                        style={{ background: `${obj.color}15`, color: obj.color }}>
                        {obj.category}
                      </div>
                      <h3 className="text-base font-semibold mb-1" style={{ color: 'var(--text-heading)' }}>
                      ...
                      <div className="flex items-center gap-1.5 text-sm font-medium opacity-0 group-hover:opacity-100 transition-opacity" style={{ color: obj.color }}>
